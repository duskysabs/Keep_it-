...
const SelectField = ({ value, defaultValue, onValueChange, options, ariaLabel, className = "", compact = false }) => {
  ...
  const selectedLabel = normalizedOptions.find((option) => option.value === currentValue)?.label || "Select";
  ...
  return <Select.Root value={currentValue} onValueChange={handleValueChange}>
    <Select.Trigger aria-label={ariaLabel} className={`flex items-center justify-between gap-3 rounded-lg border border-slate-200 bg-white text-left text-slate-700 outline-none transition hover:border-slate-300 focus:border-[#74aeb7] focus:ring-2 focus:ring-[#dceff1] ${compact ? "h-9 min-w-28 px-3 text-xs" : "h-11 w-full px-3 text-sm"} ${className}`}>
      <Select.Value>{selectedLabel}</Select.Value>
      <Select.Icon className="text-slate-400"><ChevronDown size={16} /></Select.Icon>
    </Select.Trigger>
    <Select.Portal>
      <Select.Content onEscapeKeyDown={(event) => event.stopPropagation()} position="popper" sideOffset={6} collisionPadding={12} className="z-[110] min-w-[var(--radix-select-trigger-width)] overflow-hidden rounded-lg border border-slate-200 bg-white p-1 shadow-xl">
