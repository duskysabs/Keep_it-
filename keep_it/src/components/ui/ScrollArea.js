...
const ScrollArea = ({ children, className = "", viewportClassName = "", type = "hover" }) => (
  <ScrollAreaPrimitive.Root type={type} scrollHideDelay={450} className={`relative overflow-hidden ${className}`}>
    ...
    <ScrollAreaPrimitive.Scrollbar orientation="vertical" className="flex w-2 touch-none select-none p-0.5 transition-colors">
      <ScrollAreaPrimitive.Thumb className="relative flex-1 rounded-full bg-slate-300/50 transition-colors hover:bg-slate-400/70" />
