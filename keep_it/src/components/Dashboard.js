...
const Dashboard = () => {
  ...
  const cards = [
    ["/tasks", "Open tasks", openTasks.length, overdueCount ? `${overdueCount} overdue` : "Nothing overdue", ListTodo, "bg-amber-50 text-amber-600"],
    ["/notes", "Notes", activeNotes.length, `${notes.filter((note) => note.pinned && !note.archived).length} pinned`, NotebookPen, "bg-blue-50 text-blue-600"],
    ["/vault", "Credentials", credentials.length, "Protected in your vault", KeyRound, "bg-[#e3f2f4] text-[#167d8d]"],
    ["/wallet", "Total balance", peso.format(totalBalance), `Across ${wallets.length} wallets`, WalletCards, "bg-emerald-50 text-emerald-600"],
  ];
