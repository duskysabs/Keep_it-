...
const noteDate = (value) => value ? new Intl.DateTimeFormat("en", { month: "short", day: "numeric" }).format(new Date(value)) : "Just now";
...
const Dashboard = () => {
  ...
  const vaultChecks = [
    ["Master password configured", vaultStatus.configured],
    ["Auto-lock enabled", vaultStatus.settings.autoLockMinutes > 0],
    ["Encrypted backup created", Boolean(vaultStatus.settings.backupCreatedAt)],
  ];
  const vaultScore = vaultChecks.filter(([, complete]) => complete).length;
  const vaultHealthy = vaultScore === 3;
  const cards = [
    ...
    ["/vault", "Credentials", vaultStatus.credentialCount, vaultStatus.unlocked ? "Protected in your vault" : "Vault locked", KeyRound, "bg-[#e3f2f4] text-[#167d8d]"],
  ...
  return <main className="mx-auto max-w-[1400px] p-4 sm:p-5 lg:p-8">
    ...
    <section className="mt-4 grid gap-4 xl:grid-cols-[1.25fr_0.85fr]">
      ...
      <article className="rounded-2xl border border-slate-200 bg-white p-5"><div className="flex items-center justify-between"><div><h3 className="font-bold text-slate-900">Wallet snapshot</h3><p className="mt-1 text-xs text-slate-400">This month</p></div><Link href="/wallet" className="text-xs font-bold text-[#167d8d]">Open wallet</Link></div><p className="mt-5 text-3xl font-bold text-slate-950">{peso.format(totalBalance)}</p><div className="mt-5 grid grid-cols-2 gap-3"><div className="rounded-xl bg-emerald-50 p-3"><p className="text-xs text-emerald-700">Income</p><p className="mt-1 font-bold text-emerald-800">{peso.format(monthIncome)}</p></div><div className="rounded-xl bg-rose-50 p-3"><p className="text-xs text-rose-700">Expenses</p><p className="mt-1 font-bold text-rose-800">{peso.format(monthExpenses)}</p></div></div><div className="mt-4 space-y-2">{transactions.length ? transactions.slice().sort((a, b) => b.date.localeCompare(a.date)).slice(0, 3).map((item) => { const Icon = item.type === "income" ? ArrowDownLeft : item.type === "expense" ? ArrowUpRight : ArrowLeftRight; const tone = item.type === "income" ? "bg-emerald-50 text-emerald-600" : item.type === "expense" ? "bg-rose-50 text-rose-600" : "bg-slate-100 text-slate-500"; return <Link href="/wallet" key={item.id} className="flex items-center gap-3 rounded-xl border border-slate-100 px-3 py-3 transition hover:border-[#b9dadd] hover:bg-slate-50"><span className={`grid h-8 w-8 shrink-0 place-items-center rounded-lg ${tone}`}><Icon size={15} /></span><span className="min-w-0 flex-1"><span className="block truncate text-sm font-semibold text-slate-700">{item.payee}</span><span className="mt-0.5 block text-[10px] capitalize text-slate-400">{item.type} · {shortDate(item.date)}</span></span><span className={`shrink-0 text-xs font-bold ${item.type === "income" ? "text-emerald-600" : item.type === "expense" ? "text-rose-600" : "text-slate-500"}`}>{item.type === "income" ? "+" : item.type === "expense" ? "−" : ""}{peso.format(item.amount)}</span></Link>; }) : <p className="rounded-xl border border-dashed border-slate-200 py-7 text-center text-sm text-slate-400">No transactions yet.</p>}</div></article>
    </section>

    <section className="mt-4 grid gap-4 xl:grid-cols-[1.15fr_0.95fr]">
      <article className="rounded-2xl border border-slate-200 bg-white p-5"><div className="flex items-center justify-between"><div><h3 className="font-bold text-slate-900">Pinned and recent notes</h3><p className="mt-1 text-xs text-slate-400">Continue where you left off</p></div><Link href="/notes" className="text-xs font-bold text-[#167d8d]">View all</Link></div>{dashboardNotes.length ? <div className="mt-4 grid gap-3 sm:grid-cols-3">{dashboardNotes.map((note) => <Link href="/notes" key={note.id} className="min-w-0 rounded-xl border border-slate-100 p-3 hover:bg-slate-50"><div className="flex items-center gap-2"><NotebookPen size={14} className="text-[#167d8d]" /><p className="truncate text-sm font-bold text-slate-800">{note.titleText || "Untitled note"}</p></div><p className="mt-2 line-clamp-2 text-xs leading-5 text-slate-400">{note.contentText || "Empty note"}</p><div className="mt-3 flex justify-between text-[10px] text-slate-400"><span>{note.tag}</span><span>{noteDate(note.updatedAt)}</span></div></Link>)}</div> : <p className="mt-4 rounded-xl border border-dashed border-slate-200 py-8 text-center text-sm text-slate-400">No notes yet.</p>}</article>
      <article className="rounded-2xl border border-slate-200 bg-white p-5"><div className="flex items-center justify-between"><div><h3 className="font-bold text-slate-900">Vault protection</h3><p className="mt-1 text-xs text-slate-400">Security health</p></div><ShieldCheck size={19} className={vaultHealthy ? "text-emerald-600" : "text-amber-500"} /></div><div className="mt-5 flex items-end justify-between"><p className="text-3xl font-bold text-slate-950">{vaultScore}/3</p><span className={`rounded-full px-2.5 py-1 text-xs font-bold ${vaultHealthy ? "bg-emerald-50 text-emerald-700" : "bg-amber-50 text-amber-700"}`}>{vaultHealthy ? "Healthy" : "Needs attention"}</span></div><div className="mt-3 h-2 rounded-full bg-slate-100"><div className={`h-full rounded-full ${vaultHealthy ? "bg-emerald-500" : "bg-amber-400"}`} style={{ width: `${(vaultScore / 3) * 100}%` }} /></div><div className="mt-4 space-y-2 text-xs text-slate-500">{vaultChecks.map(([label, complete]) => <p key={label} className="flex items-center gap-2">{complete ? <CheckCircle2 size={14} className="text-emerald-600" /> : <AlertTriangle size={14} className="text-amber-500" />}{label}</p>)}</div><Link href="/settings#security" className="mt-5 inline-flex text-xs font-bold text-[#167d8d]">Review security</Link></article>
    </section>
